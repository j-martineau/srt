#' @encoding UTF-8
#' @title Plot Single-Sample vs. Jackknifed Estimates of \eqn{E}
#' @inheritParams plot_C_densities
#' @return `NULL`. Called for the side effect of building and saving plots.
#' @export
plot_e_vs_E <- function(x, dir = getwd(), sub.dir = "plots", sub.sub = "e_vs_E", w = 6.5, h = 4, dpi = 500, dev = "png", clear = T) {
  uj::say("Build plots of single-sample vs. jackknifed entity capacity estimates", lev = 1)
  x <- x[x$cond == "ok" , base::c("year", "subject", "C", "E", "e")]
  ...
  base <- base::c(0, 50, 100)
  lims <- base::c(-4, 104)
  brks <- base::c(0, 25, 50, 75, 100)
  colE <- scales::alpha("cyan4"  , 2 / 3)
  colM <- scales::alpha("orange4", 2 / 3)
  pe <- ggplot2::ggplot(data = xe, mapping = ggplot2::aes(x = e, y = E))
  pm <- ggplot2::ggplot(data = xm, mapping = ggplot2::aes(x = e, y = E))
  pe <- pe + ggplot2::geom_vline(xintercept = base, color = "grey50", linetype = "dashed", linewidth = 0.15)
  pm <- pm + ggplot2::geom_vline(xintercept = base, color = "grey50", linetype = "dashed", linewidth = 0.15)
  pe <- pe + ggplot2::geom_hline(yintercept = base, color = "grey50", linetype = "dashed", linewidth = 0.15)
  pm <- pm + ggplot2::geom_hline(yintercept = base, color = "grey50", linetype = "dashed", linewidth = 0.15)
  ...
  pe <- pe + ggplot2::scale_x_continuous(name = "E (single-sample estimate)", limits = lims, breaks = brks, expand = ex)
  pm <- pm + ggplot2::scale_x_continuous(name = "E (single-sample estimate)", limits = lims, breaks = brks, expand = ex)
  pe <- pe + ggplot2::scale_y_continuous(name = "E (jackknifed estimate)"   , limits = lims, breaks = brks, expand = ex)
  pm <- pm + ggplot2::scale_y_continuous(name = "E (jackknifed estimate)"   , limits = lims, breaks = brks, expand = ex)
  pe <- pe + ggplot2::ggtitle("Single-Sample vs. Jackknifed Estimates of Entity Capacity in ELA")
  pm <- pm + ggplot2::ggtitle("Single-Sample vs. Jackknifed Estimates of Entity Capacity in Math")
  ...
  fe <- uj::p0(dir, sep, "ela_e_vs_E.", dev)
  fm <- uj::p0(dir, sep, "math_e_vs_E.", dev)
