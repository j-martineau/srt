#' @encoding UTF-8
#' @title Calculate Probability-Based Challenge Measures from Current-Year Success Indicators and Previous-Year Scaled Scores
...
#' @export
calc_Cs_P1 <- function(x, S, prev.ss, square = T, clear = T) {
  ...
  uj::check_funs("cmp_str_vec", S = S, prev.ss = prev.ss)                                                                # check [S] and [prev.ss] for complete string vec-ness
  ...
  for (SS in S) {                                                                                                        # FOR each success indicator
    lab <- uj::p0(uj::fsub(SS, "S", "C"), ".P1")                                                                         # : build label for challenge indicator
    uj::say("Calculate challenge measure [", lab, "] for success indicator [", SS, "]", lev = 3)                         # : major user update, lev 3
    CC <- uj::r(uj::nr(x), NAI)                                                                                          # : init challenge measure
    for (s in uj::suv(x$subject)) {                                                                                      # : FOR each subject
      ...
      for (y in uj::suv(x$year)) {                                                                                       # : : FOR each year
        ...
        for (g in uj::suv(x$grade)) {                                                                                    # : : : FOR each grade
          ...
          X  <- x[ix, prev.ss, drop = F]                                                                                 # : : : : logistic regression predictors are previous-year scaled scores
