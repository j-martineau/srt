...
#' @export
plot_C_densities <- function(x, Cs, conds, dir = getwd(), sub.dir = "plots", sub.sub = "C_densities", w = 6.5, h = 6, dpi = 500, dev = "png", clear = T) {
  uj::say("Calculate estimated challenge value densities", lev = 1)
  ...
  for (S in uj::suv(x$subject)) {
    ...
    for (Y in uj::suv(x$year)) {
      ...
      for (j in 1:uj::N(Cs)) {
        C    <- Cs[   j]
        cond <- conds[j]
        uj::say("subject = ", S, ", year = ", Y, ", C = ", C, lev = 2)
        iC   <- x[ , cond] == "ok"
        i    <- iS & iY & iC
        i[uj::na(i)] <- F
        CC <- uj::av(x[i , C])
        CC <- CC[uj::ok(CC)]
        CC <- stats::density(CC, cut = 1, na.rm = T)
        y <- base::rbind(y, tibble::tibble(subject = S, year = Y, C = C, x = base::c(0, CC$x, 100, 0), y = base::c(0, CC$y, 0, 0)))
      }
    }
  }
  uj::say("Build plots", lev = 1)
  levs <- base::c("C[2*A*G*P]", "C[2*P*r*o*b*1]", "C[2*P*r*o*b*2]",
                  "C[3*A*G*P]", "C[3*P*r*o*b*1]", "C[3*P*r*o*b*2]",
                  "C[4*A*G*P]", "C[4*P*r*o*b*1]", "C[4*P*r*o*b*2]")
  y$x <- base::pmin(100, base::pmax(0, y$x))
  y$C[y$C == "C2.agp"] <- "C[2*A*G*P]"
  y$C[y$C == "C3.agp"] <- "C[3*A*G*P]"
  y$C[y$C == "C4.agp"] <- "C[4*A*G*P]"
  y$C[y$C == "C2.p"  ] <- "C[2*P*r*o*b*1]"
  y$C[y$C == "C3.p"  ] <- "C[3*P*r*o*b*1]"
  y$C[y$C == "C4.p"  ] <- "C[4*P*r*o*b*1]"
  y$C[y$C == "C2.pp" ] <- "C[2*P*r*o*b*2]"
  y$C[y$C == "C3.pp" ] <- "C[3*P*r*o*b*2]"
  y$C[y$C == "C4.pp" ] <- "C[4*P*r*o*b*2]"
  ...
  pe <- pe + ggplot2::scale_x_continuous(name = "C", limits = xLims, breaks = xBrks, expand = ggplot2::expansion())
  pm <- pm + ggplot2::scale_x_continuous(name = "C", limits = xLims, breaks = xBrks, expand = ggplot2::expansion())
  pe <- pe + ggplot2::scale_y_continuous(name = "Estimated Density", expand = ggplot2::expansion(mult = 0.05))
  pm <- pm + ggplot2::scale_y_continuous(name = "Estimated Density", expand = ggplot2::expansion(mult = 0.05))
  pe <- pe + ggplot2::ggtitle("Estimated Densities of Student Challenge in ELA")
  pm <- pm + ggplot2::ggtitle("Estimated Densities of Student Challenge in Math")
  ...
  fe <- uj::p0(dir, sep, "ela_C_densities.", dev)
  fm <- uj::p0(dir, sep, "math_C_densities.", dev)
