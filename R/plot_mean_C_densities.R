...
#' @param x The input data for a call to \code{\link{calc_descriptives}}.
...
#' @export
plot_mean_C_densities <- function(x, Cs, conds, dir = getwd(), sub.dir = "plots", sub.sub = "mean_C_densities", w = 6.5, h = 4, dpi = 500, dev = "png", clear = T) {
  uj::say("Estimate mean C densities", lev = 1)
  Cs   <- uj::p0('mn.', Cs)
  ...
  for (S in uj::suv(x$subject)) {
    ...
    for (Y in uj::suv(x$year)) {
      ...
      for (j in 1:uj::N(Cs)) {
        ...
        y <- base::rbind(y, tibble::tibble(subject = S, year = Y, C = C,
                                           x = base::c(0, CC$x, 100, 0),
                                           y = base::c(0, CC$y, 0  , 0)))
      }
    }
  }
  ...
  y$C[y$C == "mn.C2.agp"] <- "C[2]^'AGP'"
  y$C[y$C == "mn.C3.agp"] <- "C[3]^'AGP'"
  y$C[y$C == "mn.C4.agp"] <- "C[4]^'AGP'"
  y$C[y$C == "mn.C2.p"  ] <- "C[2]^'P1'"
  y$C[y$C == "mn.C3.p"  ] <- "C[3]^'P1'"
  y$C[y$C == "mn.C4.p"  ] <- "C[4]^'P1'"
  y$C[y$C == "mn.C2.pp" ] <- "C[2]^'P2'"
  y$C[y$C == "mn.C3.pp" ] <- "C[3]^'P2'"
  y$C[y$C == "mn.C4.pp" ] <- "C[4]^'P2'"
  ...
  pe <- pe + ggplot2::scale_x_continuous(name = "mean(C)", limits = xLims, breaks = xBrks, expand = ggplot2::expansion())
  pm <- pm + ggplot2::scale_x_continuous(name = "mean(C)", limits = xLims, breaks = xBrks, expand = ggplot2::expansion())
  ...
  pe <- pe + ggplot2::ggtitle("Estimated Densities of Mean Student Challenge in ELA")
  pm <- pm + ggplot2::ggtitle("Estimated Densities of Mean Student Challenge in Math")
  ...
  fe <- uj::p0(dir, sep, "ela_mean_C_densities.", dev)
  fm <- uj::p0(dir, sep, "math_mean_C_densities.", dev)
